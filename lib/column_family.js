...
ColumnFamily.prototype.getKeys = function(options, callback){
  if (typeof options === 'function'){
    callback = options;
    options = {};
  }
  ...
  options.start = options.start || '';
  options.end = options.end || '';
  options.start_at = options.start_at || '';
  options.end_at = options.end_at || '';
  options.max = options.max || 200;
  ...
  var range = new ttype.KeyRange({
        start_key:options.start_at,        
        end_key:options.end_at,                
        count:options.max,
      });

  let colParent = columnParent(self);
  ...
  this.connection.execute('get_range_slices', colParent, predicate, range, consistency, onComplete);
};

ColumnFamily.prototype.multiGet = function(keys, options, callback){
  if (typeof options === 'function'){
    callback = options;
    options = {};
  }
  ...
  function onComplete(err, val){
    ...
    let results = Object.keys(val).map(rowKey => {
      let row = val[rowKey];
      return Row.fromThrift(self.keyMarshaller.deserialize(rowKey), row, self);
    })

    // var results = {}, i = 0, len = Object.keys(val).length, row;
    // for(; i < len; i += 1){
    //   row = val[i];
    //   results.push(Row.fromThrift(self.keyMarshaller.deserialize(row.key), row.columns, self));
    // }
  ...
  this.connection.execute('multiget_slice', keys, colParent, predicate, consistency, onComplete);
};
